...
export default function HomeScreen() {
  ...
  const handleDateSelectedFromHeader = useCallback(
    (newDate: Date) => {
      ...
      if (
        currentSelectedDateNormalized.getTime() !== newDateNormalized.getTime()
      ) {
  ...
  useEffect(() => {
    ...
    const selectedDateStringForComparison = formatDateToCompare(selectedDate);
  ...
  const save = async (task: UserTask) => {
    try {
      ...
      setNotification({
        message: "Ocorreu um erro ao salvar a tarefa. Tente novamente.",
        type: "error",
      });
  ...
  return (
    <PageContainer>
      ...
      >
        {!isLoading && !error && (
          <View>
            {tasksWithTime.length === 0 && tasksAnyTime.length === 0 ? (
              ...
              <>
                ...
                {tasksWithTime.length === 0 &&
                  (tasksAnyTime.length > 0 ||
                    ...
                    <Text style={styles.noTasksInCategoryText}>
